...
//!
//! This module is the git-side half of the registry publish path. It answers
//! "what commit and what artifact hash does this repo currently attest to?" by
//! walking refs, trees, and blobs -- and then hands three plain values
//! (`record_key`, `commit[20]`, `bundle[32]`) to `evm::registry_publish_record`,
//! which signs and broadcasts.
//!
//! The split matters. Before this module existed, `evm.rs` reached back into
//! `deploy`, `site`, and `object` to resolve records itself, which meant the
//! code holding the threshold-ECDSA keys transitively depended on the git
//! object store -- pack parsing, tree walking, and all -- and could not be
//! lifted into its own canister. Now the dependency points one way: git state
//! resolves here, signing happens there. See docs/CANISTER_SPLIT.md.
//!
//! Nothing here signs, and nothing in `evm.rs` reads a git object.
...
/// A resolved provenance record, ready to publish: the registry key, the
/// commit being attested, and the artifact hash bound to it.
pub struct Record {
    pub key: String,
    pub commit: [u8; 20],
    pub bundle: [u8; 32],
...
pub fn deploy_record(repo: &str, commit_oid: &Oid) -> Result<Record, String> {
    let dcfg = deploy::get_evm_config(repo)
...
pub fn site_record(repo: &str) -> Result<Record, String> {
...
pub async fn publish_commit(repo: &str, commit_oid: &Oid) -> Result<TxOutcome, String> {
    let rec = deploy_record(repo, commit_oid)?;
    evm::registry_publish_record(&rec.key, &rec.commit, &rec.bundle).await
}
